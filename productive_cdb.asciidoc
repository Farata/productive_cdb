...
First of all let's call the new project +episode_1_intro+. CDB supports different ways of linking the ExtJS framework to the application. In my case, I already have installed ExtJS libraries under my Web server (Apache Tomcat). I used +Add External Web Module+ feature of Eclipse for Java EE Developers. It allows me to map local folder with ExtJS framework files to the Tomcat web server. To do this, we need to double click on Tomcat server in Server View, switch to _Modules_ tab and click _Add External Web Module_ button. You need to specify path to the folder with ExtJS framework on your machine (_Document base:_) and web context root (_Path_). We're going to use this local ExtJS URL, but you can just specify any folder in your machine and CDB will copy the ExtJS file inside your project. Lastly, you can use ExtJS from the Sencha's CDN, if you don't want to store these libraries inside your project. Besides, using a common CDN will allow Web browser to reuse the cached version of ExtJS. 
...
As you can see Clear Data Builder generated two files as recommended by the http://martinfowler.com/dslCatalog/generationGap.html[Generation Gap pattern], which is about keeping the generated and handwritten parts separate by putting them in different classes linked by inheritance. Let's open the person model. In our case the +Person.js+ is extended from the generated +_PersonModel.js+. Should we need to customize this class, we'll do it inside the +PersonModel.js+, but this underscore-prefixed file will be regenerated each and every time when we change something in our model. CDB follows this pattern for all generated artifacts - Java services, ExtJS models and stores. This model contains all the fields from our Person DTO. 
...
<2> `getPerson` is our store fill method (the R in CRUD). We annotated this method name in `PersonService` interface with `@JSFillMethod` annotation.
