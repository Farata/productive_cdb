...
The Google Chrome Console shows that `PersonStore` was populated with one thousand records. Let's add the pagination component using the Ext `toolbarpaging` component. Let's add it to the file +sampleApp.js+ <<LISTING_3>>. In this case I'm interested in docked items. I'll add the +xtype+ `pagingtoolbar` to this component. I would like to display the information and dock this element at the top. 
...
If you'd like to repeate all of the above steps on you own watch http://faratasystems.com/training-at-farata-youtube-chanel/[the screencasts] where I perform all the actions described in this article.
