...
- Clear Data Builder - Eclipse plugin that supports code generation ExtJS MVC artifacts based on Java code.
...
<3> <vik>For this example application we can use +java.util.ArrayList+ class as in-memory server side storage of `Person` objects. In real world applications you'd use database or other persistent storage.</vik> 
...
<vik>
Ok, but my reader can ask «How to deal with the rest of CRUD methods - create, update, delete?» Let's inserts one new row, modify three existing ones and remove two.
The `Clear.data.DirectStore` object will automatically create a collection of six `ChangeObjects` - one to represent a new row, three to represent the modified ones, and two for the removed rows.

Click on the Sync menu, and the changes will be sent to corresponding remote method. When you `sync()` a standard `Ext.data.DirectStore` ExtJS is POST-ing new, modified and deleted items to the server. When the request is complete the server's reply data is applied to the store, expecting that some items can be modified by the server. 
In case of `Clear.data.DirectStore` instead of passing around items, we pass the delta, wrapped in the `ChangeObject`.
Each instance of ChangeOject contains:

- `newVersion` (or getNewVersion for Java object) of the item if we modify or create new item.
- `prevVersion` (or getPrevVersion fro Java object) of the item if modily or delete item.
- array of `changepropertyNames` if this ChangeObject describes update operation.

Rest of `ChangeObject` details described on wiki on github. Link to this could be found in _Useful Links_ section.

Corresponding Java implementation of `ChangeObject` also available on the server side. The data was processed on the server and CDB injects `ChangeObject` instance. Take a look on `getPersons_doCreate()` method. When you need to read the data that comes from the client side you need to use `changeObject.getNewVersion()`. This method will return the JSON object that you need to deserialize into the object `Person`.</vik>
...
<vik> When user clicks the button forward and backward method `loadPage` of the underlying store is called. In this case, let's examine the `directprovider` URL - server side router of remoting calls - and see how the direct request looks like.  Open Google Chrome Developer Tools from the menu View -> Developer, refresh the page and go to the Network tab. We see that each and every time when user clicks on the forward or backward buttons of pagination toolbar component sends `directOptions` as part of request. </vik>

[[FIG-9]]
.Request payload details
image::fig_09.png[image]

<vik>Clear JS - client side extension of the ExtJS framework - adds extra functionality to +Ext.data.DirectStore+ component to provide the hint to server side. At this point, these direct options can be extracted on the server side to easily get information about page's boundaries.</vik> Let's return to `PersonServiceImpl` code and let me put some extra code here.
<vik>
Right now the pagination doesn't work and we are receiving from the server one thousand records, because the server side is not aware about pagination requirements. We'll fix it in <<LISTING_7>>. </vik>
...
<2> I'm interested in +start+ and in +limit+ options that we saw on <<FIG-9>> .
...
<4> Notify the component about the total amount of the elements on the server side. For this I can use <vik> `DirectOptions.setOption()` method with +total+ option. </vik>
<5> Before returning the result, create a subset, an actual page of data. In this case I need to use method of <vik> +java.util.List.sublist()+ which produces a view of the portion of this list between indexes specified by start and limit parameters.  </vik>
...
As you can see, development of enterprise web application involves many steps that need to be done by developer. But with right set of tools repetitive steps could be and automated. Remember, don't repeat yourself! Try to do more with less effort!
I hope, you will find this article useful and helpful to start with Clear Data Builder which can tremendously improve your developer's workflow.

Also you can subscribe to our http://youtube.com/trainingAtFarata[youtube channel] to get access to latest videos, screencasts and learning materials.
